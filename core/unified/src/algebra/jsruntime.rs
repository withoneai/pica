...
impl JSRuntimeImpl {
    ...
    /// - `PicaError`: On failure, encapsulates the error details.
    ///
    /// # Errors
    ///
    ...
    pub fn create(&self, fn_name: &str, namespace: &str, code: &str) -> Result<Self, PicaError> {
        JS_RUNTIME
    ...
    pub async fn run<P, R>(&self, payload: &P, namespace: &str) -> Result<R, PicaError>
    where
        P: Serialize + Debug,
