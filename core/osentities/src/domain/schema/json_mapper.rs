...
pub fn map_data_by_schema(
    ...
    fn array(data: &Value, key: &str, path: &str, items: &Field) -> Result<Value, PicaError> {
        fn go(
            acc: &mut Vec<Value>,
            key: &str,
            items: &Field,
            rest: &[Value],
        ) -> Result<Vec<Value>, PicaError> {
            if rest.is_empty() {
                return Ok(acc.clone());
            }
                ...
                match value {
                    ...
                    Value::Object(_) => {
                        ...
                        let value = match object(data, key, &configuration, false)?.get(key) {
                            Some(arr) => Ok::<_, PicaError>(arr.clone()),
                            _ => {
                                return Err(InternalError::configuration_error(
    ...
    fn get_field_value(field: &Field, data: &Value, key: &str) -> Result<Value, PicaError> {
        match field {
            Field::String {
                required,
                default,
