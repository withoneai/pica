use crate::{InternalError, PicaError, HASH_LENGTH, HASH_PREFIX, IV_LENGTH, PASSWORD_LENGTH};
...
impl EncryptedData {
    ...
    fn decrypt(&mut self, password: &[u8; PASSWORD_LENGTH]) -> Result<&[u8], PicaError> {
    ...
    fn verify_hash(&self, password: &[u8; PASSWORD_LENGTH]) -> Result<bool, PicaError> {
        let actual_hash = Self::compute_hash(self.get_content(), self.get_iv(), password)?;
        Ok(actual_hash == *self.get_hash())
    }

    pub fn compute_hash(content: &[u8], iv: &[u8], password: &[u8]) -> Result<HashBuf, PicaError> {
